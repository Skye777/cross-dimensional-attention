...
from MultiDim_Analyzer_Model import MultiDim_Analyzer
...
class MDAnalyzer_Handler(Basement_Handler):
    def __init__(self, dataset_name, model_config, sess, is_training=True):
        ...
        self.shared_info = tf.placeholder(tf.float32, shape=(self.num_identity, self.num_shared_feature),
                                          name='position')
        ...
        self.learning_rate = tf.get_variable('learning_rate', shape=(),
                                             initializer=tf.constant_initializer(self.lr_init), trainable=False)
    ...
    def data_assignment(self, dataset_name):
        ...
        print
        'Available Segments[batches] %d[%d] Shape of data/mask piece %s and min-mean-max is %.2f-%.2f-%.2f' % (
            self.whole_segs, self.whole_size, set_whole[0].shape, disp_whole[0], disp_whole[1], disp_whole[2])
        print
        'Measurement maximum(average,std) are %4.4f(%4.4f,%4.4f)' % (self.maximum, self.scalar.mean, self.scalar.std)

        # Data Generator
        self.gen_whole = Data_Generator(self.set_whole, set_whole[0], self.whole_segs, self.batch_size, self.period_enc,
                                        is_training=True)
    ...
    def train_test_valid_assignment(self):  # , is_training = True, reuse = False
        ...
        value_sets = (
            ...
            tf.expand_dims(self.input_ori, -1),
            # the input value of decoder (future/current data with randomly removed)
        ...
        with tf.name_scope('Train'):
            with tf.variable_scope('MultiDim_Analyzer', reuse=False):
                self.MD_Analyzer_train = MultiDim_Analyzer(value_sets, self.learning_rate, self.sess, self.model_config,
                                                           is_training=True)
    ...
    def train(self):
        ...
        while epoch_cnt <= self.epochs:
            ...
            summary_format = ['loss/train_loss', 'metric/mse_segmin', 'metric/rmse', 'metric/mae', 'metric/mape',
                              'metric/mre']
